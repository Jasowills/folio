...
export default function HeaderRenderer({ data, design, style }: Props) {
  ...
  switch (style.header) {
    case 'dark-block':
      return (
        <div className="bg-ink text-white text-center py-6 px-8 -mx-[6mm] -mt-[6mm] mb-6" style={{ marginLeft: `-${6 + design.margins * 3}mm`, marginRight: `-${6 + design.margins * 3}mm`, marginTop: `-${6 + design.margins * 3}mm` }}>
          <h1 className="font-bold tracking-wide mb-1 text-[24px]" style={{ fontFamily: headingFontFamily(design) }}>
            {data.name}
          </h1>
          <div className="text-[10px] text-white/70">{contactItems.join(' \u00B7 ')}</div>
          {links.length > 0 && (
            <div className="text-[9px] text-white/60 mt-1 flex justify-center gap-2 flex-wrap">
              {links.map((l, i) => (
    ...
    case 'thin-line':
      return (
        <div className="text-center mb-5 border-b border-border pb-3">
          <h1 className="font-bold text-[22px] mb-1" style={{ fontFamily: headingFontFamily(design) }}>{data.name}</h1>
    ...
    case 'left-accent':
      return (
        <div className="text-left mb-5" style={{ borderLeft: `3px solid ${design.primaryColor}`, paddingLeft: '12px' }}>
          <h1 className="font-bold text-[22px] mb-1" style={{ fontFamily: headingFontFamily(design) }}>{data.name}</h1>
          <div className="text-[10px] text-muted">{contactItems.join(' \u00B7 ')}</div>
        </div>
      )
    ...
    case 'minimal':
      return (
        <div className="mb-5">
          <h1 className="font-bold text-[20px] mb-0.5" style={{ fontFamily: headingFontFamily(design) }}>{data.name}</h1>
          <div className="text-[10px] text-muted">{contactItems.join(' \u00B7 ')}</div>
        </div>
      )
