...
export class ResearchService {
  ...
  async getJob(jobId: string): Promise<ResearchJobDocument | null> {
    ...
    if (
      ...
      this.logger.warn(`Job ${jobId} stale in '${job.status}' state since ${updatedAt.toISOString()} — marking as failed`);
  ...
  private async runResearch(jobId: string): Promise<void> {
    ...
    if (crawledContent.length > MAX_SYNTHESIS_CHARS) {
      this.logger.log(`Truncating crawled content from ${crawledContent.length} to ${MAX_SYNTHESIS_CHARS} chars for AI synthesis`);
    ...
    try {
      ...
      if (!brief.atAGlance && !brief.whatTheyBuild) {
        throw new Error('AI response was empty or malformed. Try again or use a more specific company name.');
      }
  ...
  private async crawl(baseUrl: string, jobId?: string): Promise<Array<{ url: string; title: string; text: string }>> {
    ...
    try {
        ...
        try {
          while (pages.length < MAX_PAGES) {
            ...
            while (queue.length > 0) {
              ...
              if (normalized.includes('mailto:') || normalized.includes('tel:')) continue;
            ...
            if (jobId) {
              await this.researchJobModel
                .findByIdAndUpdate(jobId, {
                  $push: { 'crawlData.currentlyCrawling': { url: normalized, startedAt: new Date() } },
                })
                .exec();
            }

            activeCount++;
            this.logger.log(`[+${activeCount}] Crawling: ${normalized}${isSocial ? ' (social)' : ''}`);
            ...
            try {
              ...
              if (jobId) {
                await this.researchJobModel
                  .findByIdAndUpdate(jobId, {
                    $pull: { 'crawlData.currentlyCrawling': { url: normalized } },
              ...
              if (!isSocial && pages.length < MAX_PAGES) {
                ...
                this.logger.log(`  📊 queue: ${queue.length} remaining, ${pages.length} pages collected (+${activeCount} active)`);
              }
            } catch (err) {
              this.logger.warn(`  ✗ failed: ${normalized} — ${(err as Error).message}`);
              if (jobId) {
                await this.researchJobModel
                  .findByIdAndUpdate(jobId, {
                    $pull: { 'crawlData.currentlyCrawling': { url: normalized } },
      ...
      this.logger.log(`Crawl complete: ${pages.length} pages from ${domain} (${visited.size} visited)`);
      return pages;
    } catch (err) {
  ...
  private parseBrief(raw: Record<string, unknown>): ResearchJob['brief'] {
    return {
      ...
      teamSizeEstimate: raw.teamSizeEstimate ? this.safeString(raw.teamSizeEstimate) : null,
      ...
      companySizeSignal: raw.companySizeSignal ? this.safeString(raw.companySizeSignal) : null,
      ...
      roleConnection: raw.roleConnection ? this.safeString(raw.roleConnection) : null,
      recentNews: Array.isArray(raw.recentNews)
        ? (raw.recentNews as Array<{ headline: string; date: string; sourceUrl: string }>)
        : [],
      interviewStyle: {
        summary: this.safeString((raw.interviewStyle as Record<string, unknown>)?.summary),
      ...
      salaryRange: raw.salaryRange
        ? (raw.salaryRange as { estimate: string; confidence: 'high' | 'medium' | 'low' })
        : null,
    };
  }
}
