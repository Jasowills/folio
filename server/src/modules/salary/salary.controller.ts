...
export class SalaryController {
  ...
  async updateOffer(
    ...
    const data: Record<string, unknown> = { ...body, deadline: body.deadline ? new Date(body.deadline) : undefined }
    return this.salaryService.updateOffer(id, user._id.toString(), data as any)
  }

  @Delete('offers/:id')
  async deleteOffer(@CurrentUser() user: UserDocument, @Param('id') id: string) {
    await this.salaryService.deleteOffer(id, user._id.toString())
    return { success: true }
  }

  @Post('offers/:id/strategy')
  async generateStrategy(@CurrentUser() user: UserDocument, @Param('id') id: string) {
    return this.salaryService.generateStrategy(user._id.toString(), id)
  }

  @Get('offers/:id/negotiation')
  async getNegotiation(@CurrentUser() user: UserDocument, @Param('id') id: string) {
