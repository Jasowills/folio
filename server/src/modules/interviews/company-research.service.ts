...
export class CompanyResearchService {
  ...
  async research(url: string): Promise<CompanyResearchData> {
    ...
    try {
      ...
      this.logger.error(
        `Company research failed for ${url}: ${(err as Error).message}`,
      );
      return defaults;
    } finally {
  ...
  private extractInterviewStyle(text: string): string {
    ...
    if (
      ...
      return 'culture-forward';
    if (
      lower.includes('innovative') ||
      lower.includes('cutting-edge') ||
      lower.includes('technology')
    )
      return 'innovation-focused';
    if (lower.includes('customer') || lower.includes('client'))
      return 'client-centric';
    if (lower.includes('fast-paced') || lower.includes('startup'))
      return 'fast-paced';
    if (
