...
export class FollowUpService {
  ...
  constructor(
    ...
    private sessionModel: Model<InterviewSession>,
    @InjectModel(InterviewTranscript.name)
    private transcriptModel: Model<InterviewTranscript>,
    @InjectModel(InterviewResult.name)
    private resultModel: Model<InterviewResult>,
...
- tone (string): one of "warm", "professional", "enthusiastic"`;
    ...
    ) as { subject: string; body: string; tone: string };
    ...
    this.logger.log(
      `generateDraft: created follow-up ${followUp._id} for session ${sessionId}`,
    );
    return followUp;
  }
