...
export class FollowUpController {
  ...
  async getFollowUps(@CurrentUser() user: UserDocument, @Param('sessionId') sessionId: string) {
    return this.followUpService.getFollowUps(sessionId, user._id.toString())
  }

  @Post('sessions/:sessionId/follow-ups/generate')
  async generateDraft(@CurrentUser() user: UserDocument, @Param('sessionId') sessionId: string) {
  ...
  async updateFollowUp(
    ...
    @Param('id') id: string,
    @Body() body: { draftContent?: string; status?: string; sentContent?: string },
  ) {
