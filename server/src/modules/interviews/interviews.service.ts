...
export class InterviewsService {
  ...
  constructor(
    ...
    private transcriptModel: Model<InterviewTranscriptDocument>,
    @InjectModel(InterviewProctoring.name)
    private proctoringModel: Model<InterviewProctoringDocument>,
    @InjectModel(InterviewResult.name)
    private resultModel: Model<InterviewResultDocument>,
  ...
  async createSession(
    ...
    this.logger.log(
      `createSession: created session ${session._id} for role="${data.role}" level="${data.level}"`,
    );
    return session;
  }
  ...
  async generatePersona(
    ...
    if (session.interviewerPersona) {
      this.logger.log(
        `generatePersona: returning cached persona for session ${sessionId}`,
      );
      return session.interviewerPersona;
    }

    this.logger.log(
      `generatePersona: generating persona for session ${sessionId}`,
    );
    ...
    if (session.company?.url) {
      this.logger.log(
        `generatePersona: researching company at ${session.company.url}`,
      );
    ...
    const resume = session.resumeId
      ? await this.getResumeById(session.resumeId.toString())
      : null;
    ...
    if (!personaData.evaluationPriorities) {
      personaData.evaluationPriorities = [
        'communication',
        'problem solving',
        'technical depth',
      ];
    ...
    const brief = companyContext;
    const companyCtx = {
      mission: String(brief?.mission ?? ''),
      values: Array.isArray(brief?.values) ? (brief.values as string[]) : [],
      recentNews:
        Array.isArray(brief?.recentNews) &&
        (brief.recentNews as Array<Record<string, unknown>>).length > 0
          ? String(
              (brief.recentNews as Array<Record<string, unknown>>)[0]
                ?.headline ?? '',
            )
          : null,
      productFocus: String(brief?.whatTheyBuild ?? ''),
      interviewStyleSignal:
        typeof brief?.interviewStyle === 'object' &&
        brief.interviewStyle !== null
          ? String(
              (brief.interviewStyle as Record<string, unknown>)?.summary ??
                'conversational',
            )
          : 'conversational',
    };
    ...
    if (
      ...
      !persona.questionPlan ||
      persona.questionPlan.length === 0
    ) {
      throw new BadRequestException(
        'Persona generation returned incomplete data',
      );
    }
    ...
    this.logger.log(
      `generatePersona: persona "${persona.interviewerName}" generated with ${persona.questionPlan.length} questions`,
    );
    return persona;
  }
  ...
  private generateQuestionPlan(
    ...
    plans.push({
      ...
      followUpTriggers: [
        {
          condition: 'candidate mentions specific experience',
          followUp:
            'That sounds interesting. Could you tell me more about what you learned from that experience?',
        },
        {
          condition: 'candidate keeps answer brief',
          followUp:
            "I'd love to hear more about what you've been working on recently — what's been the most engaging part of your current role?",
        },
      ],
      estimatedMinutes: 3,
    ...
    plans.push({
      ...
      followUpTriggers: [
        {
          condition: 'candidate mentions team work',
          followUp:
            'How did you collaborate with others on that project? What was your role in the team dynamic?',
        },
        {
          condition: 'candidate mentions challenges',
          followUp:
            'What was the toughest challenge you faced there and how did you overcome it?',
        },
        {
          condition: 'candidate mentions results',
          followUp:
            'How did you measure the success of that project? What were the key metrics?',
        },
      ],
      estimatedMinutes: 5,
    ...
    if (tech.length > 0) {
      ...
      plans.push({
        ...
        followUpTriggers: [
          {
            condition: 'candidate describes solution',
            followUp:
              "That's a solid approach. Were there any trade-offs you had to consider?",
          },
          {
            condition: 'candidate mentions architecture',
            followUp:
              'How did you ensure the solution was scalable and maintainable?',
          },
          {
            condition: 'candidate mentions specific technology',
            followUp:
              "What made you choose that technology over alternatives you've worked with?",
          },
        ],
        estimatedMinutes: 5,
        evaluationCriteria: [
    ...
    const extraQuestions = this.generateExtraQuestions(
      session,
      extraNeeded,
      tech,
      name,
    ...
    if (company) {
      plans.push({
        ...
        followUpTriggers: [
          {
            condition: 'candidate mentions company mission',
            followUp: 'How does your personal values align with our mission?',
          },
          {
            condition: 'candidate mentions technology',
            followUp: 'What kind of impact do you hope to make in this role?',
          },
          {
            condition: 'candidate mentions team or culture',
            followUp: 'What kind of team environment do you thrive in?',
          },
        ],
        estimatedMinutes: 3,
  ...
  private generateExtraQuestions(
    ...
    templates.push({
      ...
      buildQuestion: () =>
        `Tell me about a time you faced a significant setback or failure in your work. How did you handle it, and what did you learn from the experience?`,
      followUps: [
        {
          condition: 'candidate describes failure',
          followUp: 'Looking back, what would you have done differently?',
        },
        {
          condition: 'candidate mentions learning',
          followUp: 'How has that experience changed your approach since then?',
        },
      ],
    ...
    templates.push({
      ...
      buildQuestion: () =>
        `Could you describe a situation where you had to collaborate with someone who had a very different perspective or working style? How did you navigate that?`,
      followUps: [
        {
          condition: 'candidate describes conflict',
          followUp: 'How did you find common ground? What was the outcome?',
        },
        {
          condition: 'candidate mentions compromise',
          followUp:
            'Do you feel the final solution was the best one, or just a compromise?',
        },
      ],
    ...
    templates.push({
      ...
      buildQuestion: () =>
        `Tell me about a time you went above and beyond your defined responsibilities. What motivated you, and what was the outcome?`,
      followUps: [
        {
          condition: 'candidate describes initiative',
          followUp: 'How did you get buy-in from others to pursue that?',
        },
        {
          condition: 'candidate mentions impact',
          followUp:
            'Did that initiative lead to any lasting changes in how the team operates?',
        },
      ],
    ...
    if (tech.length > 0) {
      templates.push({
        ...
        buildQuestion: () =>
          `Walk me through how you would design a system that handles ${tech[0] || 'large-scale data processing'}. What components would you consider, and what trade-offs would you make?`,
        followUps: [
          {
            condition: 'candidate describes architecture',
            followUp: 'How would you handle failure scenarios in that design?',
          },
          {
            condition: 'candidate mentions specific patterns',
            followUp:
      ...
      templates.push({
        ...
        buildQuestion: () =>
          `How do you think about code quality and maintainability? Could you share an example of how you've improved code quality in a past project?`,
        followUps: [
          {
            condition: 'candidate mentions testing',
            followUp: 'How do you decide what to test and at what level?',
          },
          {
            condition: 'candidate mentions code review',
            followUp:
      ...
      templates.push({
        ...
        buildQuestion: () =>
          `Tell me about a time you had to optimize performance in a system you worked on. How did you identify the bottleneck, and what approach did you take?`,
        followUps: [
          {
            condition: 'candidate describes optimization',
            followUp: 'How did you measure the impact of your optimization?',
          },
          {
            condition: 'candidate mentions trade-offs',
            followUp:
              'Were there any downsides to the optimization you implemented?',
          },
        ],
        criteria: [
    ...
    templates.push({
      ...
      buildQuestion: () =>
        `How do you stay current with industry trends and technologies? Can you share an example of a new skill or technology you learned recently and how you applied it?`,
      followUps: [
        {
          condition: 'candidate mentions learning method',
          followUp:
            'How do you decide what technologies are worth investing time in?',
        },
        {
          condition: 'candidate applies new skill',
          followUp:
            'What was the most challenging part of picking up that new skill?',
        },
      ],
    ...
    templates.push({
      ...
      buildQuestion: () =>
        `Tell me about a project where you had full ownership from concept to delivery. How did you prioritize what to build first, and what was the outcome?`,
      followUps: [
        {
          condition: 'candidate describes planning',
          followUp:
            'How did you handle competing priorities or scope changes along the way?',
        },
        {
          condition: 'candidate mentions stakeholders',
          followUp: 'How did you communicate progress and manage expectations?',
        },
      ],
    ...
    templates.push({
      ...
      buildQuestion: () =>
        `Can you give me an example of a time you had to explain a complex technical concept to a non-technical audience? How did you approach it?`,
      followUps: [
        {
          condition: 'candidate describes explanation',
          followUp: 'How did you know your explanation was understood?',
        },
        {
          condition: 'candidate mentions feedback',
          followUp:
            'Did you adjust your communication style based on the audience?',
        },
      ],
    ...
    if (tech.length > 0) {
      templates.push({
        ...
        buildQuestion: () =>
          `Walk me through how you approach debugging a production issue that you've never seen before. What's your process for identifying the root cause?`,
        followUps: [
          {
            condition: 'candidate describes process',
            followUp:
              'How do you decide which potential causes to investigate first?',
          },
          {
            condition: 'candidate mentions monitoring',
            followUp:
              'What kind of observability or logging would you put in place to prevent this in the future?',
          },
        ],
      ...
      templates.push({
        ...
        buildQuestion: () =>
          `Tell me about a time you had to choose between two different technical approaches or technologies. What factors did you consider, and how did you make the final decision?`,
        followUps: [
          {
            condition: 'candidate compares options',
            followUp:
              'What trade-offs did you evaluate, and how did you weigh them?',
          },
          {
            condition: 'candidate mentions team input',
            followUp: 'How did you get buy-in from the team on your decision?',
          },
        ],
    ...
    templates.push({
      ...
      buildQuestion: () =>
        `Describe a situation where you had to make a decision with incomplete information. How did you approach it, and what was the result?`,
      followUps: [
        {
          condition: 'candidate describes decision process',
          followUp:
            'How did you validate you were heading in the right direction?',
        },
        {
          condition: 'candidate mentions risk',
          followUp:
            'How do you think about risk when making technical decisions?',
        },
      ],
  ...
  async updateLastCandidateTurnSatisfaction(
    sessionId: string,
  ...
  buildGreeting(
    ...
    const persona = session.interviewerPersona;
  ...
  private async generateResults(
    ...
    try {
      ...
      this.logger.error(
        `generateResults: AI scoring failed — ${(err as Error).message}`,
      );
      await this.resultModel.create({
        sessionId,
        overallScore: 0,
