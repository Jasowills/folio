...
export class SubmitWorkerService {
  ...
  async submit(
    ...
    try {
      ...
      const successText =
        (await page.textContent('body').catch(() => undefined)) ?? '';
      ...
      const hasSuccess = successIndicators.some((s) =>
        successText.toLowerCase().includes(s),
      );
      ...
      const errorText = await page
        .textContent('.error, .alert, .notification--error, [class*="error"]')
        .catch(() => '');
      if (errorText) {
        return {
          success: false,
          error: `Form error: ${errorText.trim().slice(0, 300)}`,
        };
      }

      return {
        success: false,
        error: 'Unknown result — no success message detected',
      };
    } catch (err) {
    ...
    for (const field of fields) {
      ...
      try {
        ...
        for (const sel of selectors) {
          ...
          if (el) {
            const tag = await el
              .evaluate((node: Element) => node.tagName.toLowerCase())
              .catch(() => '');
            if (tag === 'textarea') {
              await el.fill(value);
            } else if (tag === 'select') {
  ...
  private async findSubmitButton(
    ...
    switch (platform) {
      ...
      case 'workday':
        return page.$(
          'button[data-automation-id*="submit"], button[aria-label*="Submit"], button:has-text("Submit")',
        );
      case 'icims':
        return page.$('input[type="submit"], button[type="submit"]');
      case 'ashby':
        return page.$(
          'button[data-cy*="submit"], button:has-text("Submit application"), button:has-text("Submit")',
        );
      default:
