...
import { ApplySubmission, ApplySubmissionDocument, ApplyField } from './schemas/apply-submission.schema';
import { AnswersBankEntry, AnswersBankDocument } from './schemas/answers-bank.schema';
import { AutoApplyConfig, AutoApplyConfigDocument } from './schemas/auto-apply-config.schema';
...
export class ApplyFlowService {
  ...
  constructor(
    @InjectModel(ApplySubmission.name) private submissionModel: Model<ApplySubmissionDocument>,
    @InjectModel(AnswersBankEntry.name) private answersBankModel: Model<AnswersBankDocument>,
    @InjectModel(AutoApplyConfig.name) private configModel: Model<AutoApplyConfigDocument>,
  ...
  async approveJobs(
    ...
    if (!complete) {
      throw new ForbiddenException('Auto-apply setup is not complete. Finish the setup wizard first.');
    }
    ...
    if (!effectiveResumeId) {
      ...
      if (!resumes?.length) throw new BadRequestException('No resume found. Upload a resume first.');
      effectiveResumeId = resumes[0]._id.toString();
    }
    const resumeObjId = new Types.ObjectId(effectiveResumeId);
    const resume = await this.resumesService.findById(effectiveResumeId, userId);
    const userData = {
    ...
    for (const jobId of jobIds) {
      ...
      if (atsPlatform === 'unknown') {
        this.logger.warn(`Unknown ATS for job ${jobId}, URL: ${applicationUrl}`);
      }

      let coverLetterId: Types.ObjectId | undefined;
      if (generateCoverLetter) {
        const config = await this.configModel.findOne({ userId: userObjId }).exec()
        ...
        if (config.autoAttachCoverLetter !== false) {
          try {
            ...
            coverLetterId = coverLetter._id as Types.ObjectId;
          } catch (err) {
            this.logger.debug(`Cover letter generation failed for job ${jobId}:`, err);
          }
        }
      }
      ...
      const submission = await this.submissionModel.create({
        ...
        applicationUrl,
        retryCount: 0,
        activityLog: [{ action: 'Approved for auto-apply', timestamp: new Date() }],
      });
  ...
  async fillApplication(userId: string, submissionId: string): Promise<ApplySubmissionDocument> {
    ...
    if (submission.status !== 'approved') {
      throw new BadRequestException(`Cannot fill submission in status: ${submission.status}`);
    }

    const resume = await this.resumesService.findById(submission.resumeId.toString(), userId);

    const config = await this.configModel.findOne({ userId: new Types.ObjectId(userId) }).exec();
    ...
    const job = await this.jobListingModel.findById(submission.jobListingId).exec();
    ...
    if (submission.coverLetterId) {
      try {
        const cl = await this.coverLettersService.findById(submission.coverLetterId.toString(), userId);
    ...
    if (!result.success) {
      ...
      submission.activityLog.push({ action: `Failed: ${submission.failureReason}`, timestamp: new Date() });
    ...
    if (result.screeningQuestions.length > 0 && job) {
      const answered = await this.screeningAgent.answerQuestions(userId, result.screeningQuestions, {
        resumeData: {
          name: resume.name ?? undefined,
          summary: config?.personalSummary || (resume.summary ?? undefined),
          skills: resume.skills?.filter((s): s is string => s != null) ?? undefined,
          ...
          contact: resume.contact
            ? {
                ...
                location: config?.preferredLocation || (resume.contact.location ?? undefined),
              }
            : undefined,
        },
        jobData: {
  ...
  async getPreview(userId: string, submissionId: string): Promise<ApplySubmissionDocument> {
    const submission = await this.submissionModel.findOne({
      _id: new Types.ObjectId(submissionId),
      userId: new Types.ObjectId(userId),
    }).populate('jobListingId').exec();
    if (!submission) throw new NotFoundException('Submission not found');
    return submission;
  }
  ...
  async confirmAndSubmit(
    ...
    if (submission.status !== 'ready_for_review') {
      throw new BadRequestException(`Cannot submit in status: ${submission.status}. Must be ready_for_review.`);
    }

    if (updatedFields && updatedFields.length > 0) {
      const fieldMap = new Map(submission.filledFields?.map((f) => [f.fieldName, f]) || []);
    ...
    submission.activityLog.push({ action: 'Confirmed by user - submitting', timestamp: new Date() });
    await submission.save();
    ...
    if (result.success) {
      ...
      submission.activityLog.push({ action: 'Application submitted successfully', timestamp: new Date() });

      await this.updateTrackerJobStage(userId, submission.jobListingId.toString(), 'submitted');
    } else {
      submission.status = 'failed';
      submission.failureReason = result.error || 'Submission failed';
      submission.retryCount = (submission.retryCount || 0) + 1;
      submission.activityLog.push({ action: `Submission failed: ${submission.failureReason}`, timestamp: new Date() });
    }
  ...
  async getSubmissions(userId: string): Promise<ApplySubmissionDocument[]> {
    return this.submissionModel.find({
      userId: new Types.ObjectId(userId),
    }).populate('jobListingId').sort({ createdAt: -1 }).exec();
  }

  async getSubmissionById(userId: string, submissionId: string): Promise<ApplySubmissionDocument> {
    const submission = await this.submissionModel.findOne({
      _id: new Types.ObjectId(submissionId),
      userId: new Types.ObjectId(userId),
    }).populate('jobListingId').exec();
  ...
  async retrySubmission(userId: string, submissionId: string): Promise<ApplySubmissionDocument> {
    const submission = await this.submissionModel.findOne({
      _id: new Types.ObjectId(submissionId),
      userId: new Types.ObjectId(userId),
    }).exec();
    ...
    if (submission.status !== 'failed') {
      throw new BadRequestException(`Can only retry failed submissions, got: ${submission.status}`);
    }

    submission.status = 'approved';
    submission.failureReason = undefined;
    submission.activityLog.push({ action: 'Retry requested', timestamp: new Date() });
    await submission.save();
  ...
  async getAnswersBank(userId: string): Promise<AnswersBankEntry[]> {
    return this.answersBankModel.find({ userId: new Types.ObjectId(userId) }).exec();
  }

  async storeAnswer(userId: string, question: string, answer: string, category: string): Promise<AnswersBankEntry> {
  ...
  async deleteAnswer(userId: string, answerId: string): Promise<void> {
    const result = await this.answersBankModel.deleteOne({
  ...
  async getConfig(userId: string): Promise<AutoApplyConfigDocument | null> {
    return this.configModel.findOne({ userId: new Types.ObjectId(userId) }).exec();
  }

  async upsertConfig(userId: string, updates: Record<string, any>): Promise<AutoApplyConfigDocument> {
  ...
  private async updateTrackerJobStage(userId: string, jobListingId: string, stage: string): Promise<void> {
    try {
      await this.jobAppModel.updateOne(
        { userId: new Types.ObjectId(userId), jobListingId: new Types.ObjectId(jobListingId) },
        { $set: { stage, lastActivityAt: new Date() } },
      ).exec();
    } catch (err) {
      this.logger.debug('Could not update tracker stage:', (err as Error).message);
    }
  }
}
