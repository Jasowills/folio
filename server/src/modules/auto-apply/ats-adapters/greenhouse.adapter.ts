  ...
  async fillApplication(
    ...
    try {
      ...
      if (userData?.fullName) {
        ...
        if (filled)
          fields.push({
            ...
            fieldValue: userData.fullName.split(' ')[0] || '',
            autoFilled: true,
            editable: true,
          });
        const filledLast = await this.fillField(
          page,
        ...
        if (filledLast)
          fields.push({
            fieldName: 'last_name',
            fieldValue: userData.fullName.split(' ').slice(1).join(' ') || '',
            autoFilled: true,
            editable: true,
          });
      }
      ...
      if (userData?.linkedinUrl) {
        const filled = await this.fillField(
          page,
          'input#job_application_linkedin_url',
      ...
      if (userData?.website) {
        const filled = await this.fillField(
          page,
          'input#job_application_website_url',
          userData.website,
        );
        if (filled)
          fields.push({
      ...
      const questionElements = await page.$$(
        '.application-question, .field, .form-group, [class*="question"]',
      );
  ...
  async submitApplication(page: Page): Promise<boolean> {
    try {
      const submitBtn = await page.$(
        'button[type="submit"], input[type="submit"], .submit-button, #submit_app',
      );
