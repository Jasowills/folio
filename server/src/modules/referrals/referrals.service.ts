...
import { ReferralRequest, ReferralRequestDocument } from './schemas/referral-request.schema'
...
export class ReferralsService {
  ...
  constructor(
    ...
    @InjectModel(ReferralRequest.name) private referralRequestModel: Model<ReferralRequestDocument>,
  ...
  async importConnections(
    userId: string,
    rows: Array<{ firstName: string; lastName?: string; email?: string; companyName?: string; position?: string; connectedOn?: string }>,
  ) {
    ...
    const result = await this.connectionModel.insertMany(docs, { ordered: false })
    this.logger.log(`importConnections: imported ${result.length} connections for user ${userId}`)
    return result
  }
  ...
  async findReferrals(userId: string, jobListingId?: string) {
    ...
    const jobs = await this.jobListingModel.find(jobFilter).select('companyName roleTitle location _id').exec()
    if (!jobs.length) return []

    const jobCompanyNames = [...new Set(jobs.map((j) => j.companyName.toLowerCase()))]

    const connections = await this.connectionModel.find({ userId, status: 'active' }).exec()
  ...
  async generateReferralMessage(userId: string, connectionId: string, jobListingId: string) {
    const existing = await this.referralRequestModel.findOne({ userId, connectionId, jobListingId }).exec()
    if (existing?.draftMessage) return existing

    const connection = await this.connectionModel.findOne({ _id: connectionId, userId }).exec()
...
- body (string): short, respectful, mentions the connection's work at the company, explains why the candidate is a good fit, asks if they'd be comfortable providing a referral`,
    ...
    const referral = existing
      ? await this.referralRequestModel
          .findByIdAndUpdate(existing._id, { $set: { draftMessage } }, { new: true })
          .exec()
      : await this.referralRequestModel.create({
          userId, connectionId, jobListingId, draftMessage, status: 'draft',
        })
  ...
  async getReferralRequests(userId: string, status?: string) {
    const filter: Record<string, unknown> = { userId }
    if (status) filter.status = status
    return this.referralRequestModel.find(filter).sort({ createdAt: -1 }).exec()
  }

  async updateReferralRequest(id: string, userId: string, body: Partial<{ status: string; sentMessage: string; notes: string }>) {
  ...
  async deleteConnection(id: string, userId: string) {
    const conn = await this.connectionModel.findOneAndDelete({ _id: id, userId }).exec()
